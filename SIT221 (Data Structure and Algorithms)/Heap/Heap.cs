...
namespace Heap
{
    public class Heap<K, D> where K : IComparable<K>
    {
        ...
        public IHeapifyable<K, D>[] BuildHeap(K[] keys, D[] data)
        {
            ...
            {
                Node newNode = new Node(keys[i], data[i], i);
                result[i] = newNode;
            }
            return result;
        }
        ...
        {
            int root_temp = root; // Initialize largest as root  
            int l = 2 * root_temp + 1; // left = 2*i + 1  
            int r = 2 * root_temp + 2; // right = 2*i + 2  
   
            if (l < Count && comparer.Compare(data[root_temp].Key, data[l].Key) > 0)
            {
                root_temp = l;
            }  
  
            if (r < Count && comparer.Compare(data[root_temp].Key, data[r].Key) > 0) 
            {
                root_temp = r;
            }  
        ...
        public void heapify_build(Node[] arr, int n, int i)
        {
            int parent = (i - 1) / 2;
            if (arr[parent].Data > 0)
            {
                if (comparer.Compare(data[i].Key, data[parent].Key) > 0)
                {
                    Node temp = arr[i];
                    arr[]
                }
